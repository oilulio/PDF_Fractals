...
TITLE="Julia Set"
AUTHOR="S Combes"
XSHIFT="0.0"  # note strings
SCALE="1.0" # Works on the default A4

C={"X":"-0.8","Y":"0.156"}
#C={"X":"-0.7","Y":"0.27015"}

DEPTH=32   # Equipotentials to show.  Don't exceed 32 if using colours
SQ_THRESHOLD="4" # Squared distance, as string, considered to show out of set
...
for i in range(DEPTH):
  stream.append(" "*i+"{")
  # Several evolutions per test as Julia grows slowly
  for j in range(5):
    stream.append(" "*i+" 2 copy 2 mul mul "+C["Y"]+" add 3 1 roll dup mul exch dup mul sub "+C["X"]+" add ") 
  stream.append(" "*i+" 2 copy 2 mul mul "+C["Y"]+" add 3 1 roll dup mul exch dup mul sub "+C["X"]+" add 2 copy dup mul exch dup mul add")
  if (i%2==0):
    colR=0
    colG=0
    colB=0
